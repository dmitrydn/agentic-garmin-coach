...
7. При флаге volume_over в context_flags → снизить объём сегодня (короче длительность,
   не интенсивность). При volume_under: держать план; и ТОЛЬКО если выполнены ВСЕ условия —
   readiness нормальный/высокий, НЕТ ни одного injury/achilles/hrv/acwr/vertical-флага,
   до A-гонки >14 дней (НЕ тейпер), и сегодня лёгкий/базовый день — можно консервативно
   добавить +10-15% ДЛИТЕЛЬНОСТИ (только объём, НЕ интенсивность и НЕ вертикаль), чтобы
   подтянуть недельный дефицит. Иначе держать без изменений. Дефицит НИКОГДА не догонять
   интенсивностью, вертикалью или в тейпере — это путь к рецидиву травмы.
8. При флагах vertical_spike_high / vertical_spike_caution / vertical_over в context_flags →
   сегодня БЕЗ вертикали (плоский профиль) ИЛИ подъёмы powerhike + спуски пешком; срезать
   плановый D+. Резкий набор вертикали — доказанный триггер травмы ахилла у этого атлета.
   НИКОГДА не добавлять вертикаль/D+ при любом vertical- или ахилл-флаге, даже при высоком readiness.
